...
class KontrakujiController extends Controller
{
    ...
    public function indexlight(Request $request)
    {
        try {
            ...
            $v = ConditionContractNew::with([
                ...
                'kontrakuji.contract_attachment',
            ...
            if (!is_null($data['tgl_selesai'])) {
                ...
                $v = $v->whereIn('condition_contracts.contract_id', [\DB::raw('SELECT id_contract FROM transaction_sample WHERE DATE_FORMAT(tgl_selesai,"%Y-%m-%d") ="' . $data['tgl_selesai'] . '" GROUP BY id_contract')]);
            }
            ...
            if ($data['statuscontract'] > 0) {
                $v = $v->where('status', $data['statuscontract'] - 1);
            }
    ...
    public function cekidot(Request $request)
    {
        try {

            $v = ConditionCert::whereIn('status', [4, 3])->get()->toArray();
        } catch (\Exception $e) {
            return response()->json($e->getMessage());
        }
    }

    public function checkconditioncs(Request $request)
    {
        try {

            $var = \DB::table('condition_contracts')
                ->where('position', 1)
                ->where('contract_id', $request->input('id_contract'))
                ->get();
    ...
    public function indexlightfinder(Request $request)
    {
        try {
            ...
            $o = ConditionContractNew::with([
                'kontrakujifinder.condition_certOne',
                'kontrakujifinder.payment_condition',
                'user'
            ])
                ->selectRaw('
                user_id,
            ...
            if (!empty($data['search'])) {
                ...
                if (count($samplename) < 1) {
                    ...
                    if (count($check) > 0) {
                        ...
                        $df = array_map(function ($e) {
                            return $e->id_contract;
                        }, $check);

                        $o = $o->whereIn('contract_id', $df);
                    } else {
                        $o = $o->whereHas('kontrakujifinder', function ($q) use ($data) {
                            return $q->where(\DB::raw('UPPER(contract_no)'), 'like', '%' . trim(strtoupper($data['search'])) . '%');
                        });
                    }
                } else {
                    ...
                    $o = $o->whereIn('contract_id', $azz);
                }
            }

            if (!is_null($data['contract_category'])) {
                if ($data['contract_category'] > 0) {
                    $o = $o->whereHas('kontrakujifinder', function ($z) use ($data) {
                        return $z->where('id_contract_category', $data['contract_category']);
                    });
                }
            }
            ...
            if (!empty($data['no_po'])) {
                if ($data['no_po'] !== '-') {
                    $o = $o->whereHas('kontrakujifinder', function ($r) use ($data) {
                        return $r->where('no_po', $data['no_po']);
                    });
                }
            }
            ...
            if (!empty($data['no_penawaran'])) {
                if ($data['no_penawaran'] !== '-') {
                    ...
                    $g = \DB::table('transaction_penawaran as z')
                        ->where('z.no_penawaran', $data['no_penawaran'])
                        ->first();

                    $o = $o->whereHas('kontrakujifinder', function ($r) use ($data, $g) {
                        if ($g) {
                            return $r->where('id_penawaran', $g->id);
                        } else {
                            return $r->where('no_penawaran', $data['no_penawaran']);
                        }
                    });
                }
            }
            ...
            if (!empty($data['user_created'])) {
                if ($data['user_created'] > 0) {
                    $o = $o->where('user_id', $data['user_created']);
                }
            }
            ...
            if (!empty($data['statuskontrak'])) {
                if ($data['statuskontrak'] !== 'all') {
                    if ($data['statuskontrak'] == 1) {
                        ...
                        $fd = \DB::connection('mysqlcertificate')
                            ->table('condition_sample_cert as az')
                            ->select('az.id_contract')
                            ->whereBetween(\DB::raw('az.inserted_at'), [$year . '-01-01', $year . '-12-31'])
                        ...
                        $o = $o->where('status', 1)->whereNotIn('contract_id', $zsd);
                        ...
                        $asd = array_map(function ($xda) {
                            return $xda->id_contract;
                        }, $fd);

                        $o = $o->where('status', 1)->whereIn('contract_id', $asd);
            ...
            if (!is_null($data['customers'])) {
                if ($data['customers'] > 0) {
                    $v = \DB::table('mstr_customers_handle as a')
                        ->join('mstr_transaction_kontrakuji as b', 'b.id_customers_handle', 'a.idch')
                        ->select('b.id_kontrakuji')
                        ->where('id_customer', $data['customers'])
                        ->get()->toArray();

                    $za = array_map(function ($dab) {
                        return $dab->id_kontrakuji;
                    }, $v);
            ...
            if (!empty($data['pic'])) {
                $va = \DB::table('mstr_transaction_kontrakuji')
                    ->where('status', $data['pic'])
                    ->get()->toArray();

                $zab = array_map(function ($dan) {
                    return $dan->id_kontrakuji;
                }, $va);

                $o = $o->whereIn('contract_id', $zab);
            }

            if (!is_null($data['tgl_selesai'])) {
                $y = \DB::table('transaction_sample as a')
    ...
    public function index(Request $request)
    {
        ...
        if (
            ...
            $request->input('month') == 0 && ($request->input('no_penawaran') == 'undefined' || $request->input('no_penawaran') == '-') && ($request->input('no_po') == 'undefined' || $request->input('no_po') == '-') &&
    ...
    public function getnopo(Request $request)
    {
        ...
        if ($request->has('search')) {
            $v = $v->where(\DB::raw('UPPER(a.no_po)'), 'like', '%' . strtoupper($request->input('search')) . '%');
        }
        $v = $v->whereNotNull('a.no_po')
            ->where('a.no_po', '<>', '')
    ...
    public function getnopenawaran(Request $request)
    {
        ...
        if ($request->has('search')) {
            $v = $v->where(\DB::raw('UPPER(a.no_penawaran)'), 'like', '%' . strtoupper($request->input('search')) . '%');
        }

        $v = $v->whereNotNull('a.no_penawaran')
            ->where('a.no_penawaran', '<>', '')
            ->orderBy('a.created_at', 'desc')
            ->groupBy('a.no_penawaran')
            ->paginate(25)->toArray();

        $r = \DB::table('transaction_penawaran as b')
            ->select('b.no_penawaran');

        if ($request->has('search')) {
            $r = $r->where('b.no_penawaran', 'like', '%' . $request->input('search') . '%');
        }

        $r = $r->paginate(25)->toArray();

        $g = array_merge($r['data'], $v['data']);
    ...
    public function edit_data_contract(Request $request, $id)
    {
        try {
            ...
            if (!empty($data['penawaran'])) {
                $v->id_penawaran = $data['penawaran'];
            }
            $v->id_alamat_customer = $data['alamatcustomer'];
            $v->status = !empty($data['clienthandling']) ? $data['clienthandling'] : NULL;
            $v->save();
            ...
            if (!empty($data['memo_finance'])) {
                $g = Description::where('id_contract', $id)->where('status', 1)->where('groups', 6)->get();
                foreach ($g as $f) {
                    $x = Description::find($f->id)->delete();
                }
                $d = new Description;
    ...
    public function edit_pic_contract(Request $request)
    {
        try {

            $var = Kontrakuji::find($request->input('contract_id'));
            $var->status = $request->input('sales_id');
            $var->save();
    ...
    public function sampleinformationedit(Request $request, $id)
    {
        ...
        $var = TransactionSample::find($id);
        $var->kode_sample = $data['kode_sample'];
        $var->batch_number = $data['batch_number'];
        $var->lot_number = $data['lot_number'];
        $var->nama_dagang = $data['nama_dagang'];
        $var->nama_pabrik = $data['nama_pabrik'];
        $var->alamat_pabrik = $data['alamat_pabrik'];
        $var->no_notifikasi = $data['no_notifikasi'];
        $var->no_pengajuan = $data['no_pengajuan'];
        $var->no_registrasi = $data['no_registrasi'];
        $var->no_principalcode = $data['no_principalcode'];
        $var->keterangan_lain = $data['keterangan_lain'];
        $var->jenis_kemasan = $data['jenis_kemasan'];
        $var->tgl_produksi = $data['tgl_produksi'];
        $var->tgl_kadaluarsa = $data['tgl_kadaluarsa'];
        $var->save();
    ...
    public function revision(Request $request, $id)
    {
        ...
        foreach ($data['sample'] as $smple) {
            ...
            if (isset($smple['detsample'])) {
                ...
                $csample->kode_sample =  !empty($smpledet['kode_sample']) ? $smpledet['kode_sample'] : null;
                if ($revcontract->id_contract_category !== $data['contractcategory']) {
                    $z = ContractCategory::find($data['contractcategory']);
                    $checklastval = \DB::select('SELECT
                        IFNULL(MAX(CAST(IF(SUBSTRING_INDEX(SUBSTRING_INDEX(a.no_sample,".",-2),".",1) = "REV",SUBSTRING_INDEX(SUBSTRING_INDEX(a.no_sample,".",-3),".",1),SUBSTRING_INDEX(a.no_sample,".",-1)) AS SIGNED)),0) AS max
        ...
        if (!empty($data['penawaran'])) {
            $revcontract->id_penawaran = $data['penawaran'];
        }
        ...
        // if( $data['discount_conv'] > 5){
        //     $this->sendingEmail($data, $id_user, $revcontract, $editp, "Merevisi Pengujian");
        // }
    ...
    public function acceptContractNew(Request $request)
    {
        try {
            ...
            $var = ConditionContractNew::whereIn('contract_id', $data)
                ->where('position', 1)
                ->where('groups', 'CS')
                ->where('status', '<>', 5)
                ->get();
            ...
            foreach ($var as $v) {
                $g = ConditionContractNew::find($v->id_condition_contract);
                $g->status = 1;
                $g->save();

                $cj = new ContractMessage;
                $cj->id_contract = $v->contract_id;
                $cj->user_id = $id_user;
                $cj->status = 4;
                ...
                $acceptConditionKendali->contract_id = $v->contract_id;
                $acceptConditionKendali->sample_id = $v->sample_id;
                $acceptConditionKendali->parameter_id = $v->parameter_id;
                $acceptConditionKendali->inserted_at = time::now();
                $acceptConditionKendali->user_id = $v->user_id;
    ...
    public function store(Request $request)
    {
        try {
            ...
            $kontrak->status = !empty($data['clienthandling']) ? $data['clienthandling'] : null;
            $kontrak->id_alamat_customer = $data['alamatcustomer'];
            $kontrak->id_penawaran = !empty($data['penawaran']) ? $data['penawaran'] : null;
            $kontrak->id_contract_category = $data['contract_category'];
            $kontrak->created_at = time::now();
            $kontrak->save();

            $g = \DB::table('mstr_customers_handle as a')
                ->join('mstr_customers_customer as b', 'b.id_customer', 'a.id_customer')
                ->where('a.idch', $data['customerhandle'])
                ->groupBy('b.id_customer')
                ->first();

            $checknpwp = \DB::table('mstr_customers_handle as a')
                ->join('mstr_customers_customer as b', 'b.id_customer', 'a.id_customer')
                ->join('master_npwp as c', 'c.id_customer', 'b.id_customer')
                ->where('a.idch', $data['customerhandle'])
                ->get();


            if ($g) {
                if ($g->status_invoice == 1) {
                    $h = Kontrakuji::find($kontrak->id_kontrakuji);
                    $h->hold = 'Y';
                    $h->save();
                } else {
            ...
            if (!empty($data['memo_finance'])) {
                $dmemo = new Description;
                $dmemo->id_contract = $kontrak->id_kontrakuji;
                $dmemo->id_sample = 0;
                $dmemo->id_parameter = 0;
                $dmemo->desc = $data['memo_finance'];
                $dmemo->status = 1;
                $dmemo->groups = 6;
                $dmemo->insert_user = $id_user;
                $dmemo->created_at = time::now();
                $dmemo->save();
            }
            ...
            foreach ($data['sample'] as $key => $sample) {
                ...
                $a->sample_name = $sample['samplename'];
                $a->kode_sample = !empty($sample['parameter']['kodesample']) ? (!empty($sample['parameter']['kodesample']) ? $sample['parameter']['kodesample'] : $sample['parameter']['kode_sample']) : null;
                $a->id_tujuanpengujian = $sample['parameter']['tujuanpengujian'];
                $a->id_statuspengujian = $sample['parameter']['statuspengujian'];
                $a->batch_number = !empty($sample['parameter']['batchno']) ? $sample['parameter']['batchno'] : null;
                $a->lot_number = $sample['parameter']['lotno'];
                ...
                if (count($sample['parameter']['paketparameter']) > 0) {
                    ...
                    foreach ($sample['parameter']['paketparameter'] as $paketparameter) {
                        ...
                        foreach ($paketparameter['paketparameter'] as $c) {
                            ...
                            $b->id_lab = $c['id_lab'];
                            $b->id_metode = $c['id_metode'];
                            $b->id_unit = !empty($c['id_unit']) ? $c['id_unit'] : $c['id_satuan'];
                            $b->disc_parameter = $paketparameter['discount'];
                            $b->id_standart = $c['id_standart'];
                            $b->format_hasil = NULL;
                            $b->status = 1;
                ...
                if (count($sample['parameter']['nonpaketparameter']) > 0) {
                    foreach ($sample['parameter']['nonpaketparameter'] as $satuanparameter) {
                        ...
                        $x->id_sample = $a->id;
                        $x->disc_parameter = $a->discount;
                        $x->format_hasil =  !empty($satuanparameter['formathasil']) ? $satuanparameter['formathasil'] : "Simplo";
                        $x->status = 2;
            ...
            $payment->biaya_pengujian = $data['totalpembayaran'];
            $payment->discount_lepas = $data['hasilDiscount'];
            $payment->disc_val = $data['discount'];
            $payment->ppn = $data['ppn'];
            $payment->downpayment = $data['uangmuka'];
            ...
            // $chekcust = \DB::table('mstr_customers_handle as a')->where('a.idch',$kontrak->id_customers_handle)->first();
            ...
            //         if($data['discount'] > 0){
            //             $a = [52, 67];
            //             $this->sendingEmail($data, $id_user, $kontrak, $a);

            //             $conditioncontractupdating = ConditionContractNew::find($conditioncontract->id_condition_contract);
            //             $conditioncontractupdating->status = 2;
            //             $conditioncontractupdating->save();
            //         }
            ...
            // if($data['discount'] > 5 && $data['typeContract'] !== 4){
            //     $this->sendingEmail($data, $id_user, $kontrak, $payment, "Membuat pengujian baru");
            // }
    ...
    private function sendingEmail($data, $id_user, $kontrak, $st)
    {
        $ems = Employee::with(['user'])->where('user_id', $id_user)->first();

        $buatemail = array(
            "contract_no" => $kontrak->contract_no,
            "id_kontrakuji" => $kontrak->id_kontrakuji,
            "discount" => $data['discount'],
            'harga_discount' => $data['hasilDiscount'],
            "totalharga" => $data['totalpembayaran'],
            "employee" => $ems,
            "statusmessage" => 'Menambah Kontrakuji',
            'st' => 'kontrakuji',
            'url' => 'https://dummy.siglab.co.id/#/analystpro/view-contract'
        );

        Mail::send('Kontrak/approving', $buatemail, function ($message) use ($id_user, $kontrak, $data) {

            $message->to('herdinop.sig@saraswanti.com')->subject('Contract Review, ' . $kontrak->contract_no);
    ...
    public function acceptcontractdisc(Request $request)
    {
        ...
        $v = ConditionContractNew::where('contract_id', $request->input('idcontract'))
            ->where('position', 1)
            ->first();

        $change = ConditionContractNew::find($v->id_condition_contract);
        $change->status = 1;
        $change->save();


        $setmessage = new ContractMessage;
        $setmessage->id_contract = $request->input('idcontract');
        $setmessage->user_id = $id_user;
        $setmessage->status = 4;
        $setmessage->message = "Approving Contract with discount more than 10%";
        $setmessage->inserted_at = time::now();
        $setmessage->save();

        return response()->json(array(
            "status" => true,
            "message" => "Success Approving Contract"
        ));
    }
    ...
    public function sendAttachmentBlob(Request $request)
    {
        ...
        $idc = \DB::table('mstr_transaction_kontrakuji as a')->where('a.contract_no', $data['contract_no'])->first();

        $attachmentContract = ContractAttachment::where('id_contract', $idc->id_kontrakuji)->get();
        ...
        $setAttachment->id_contract = $idc->id_kontrakuji;
    ...
    public function hargaanehpaket(Request $request)
    {
        try {

            $var = \DB::table('harga_aneh')->get();
    ...
    public function excelForSales(Request $request)
    {
        try {
            ...
            $z = $u->id_bagian !== 13 ? "" : "AND c.contract_type = 4";
            $var = \DB::select('
                SELECT 
                g.customer_name,
                h.name AS cp,
                i.employee_name,
                c.contract_no,
                sampleprice.tgl_input,
                sampleprice.tgl_selesai,
                c.`desc`,
                c.no_po,
                (SELECT employee_name FROM hris_employee where employee_id = c.status) as pic_sales,
                c.no_penawaran,
                ...
                LEFT JOIN (SELECT SUM(b.total) AS total,b.id_transaction_contract FROM transaction_sampling_contract b GROUP BY b.id_transaction_contract) AS sampling ON sampling.id_transaction_contract = a.contract_id
                LEFT JOIN (SELECT SUM(b.total) AS total,b.id_transaction_kontrakuji FROM transaction_akg_contract b GROUP BY b.id_transaction_kontrakuji) AS akg ON akg.id_transaction_kontrakuji = a.contract_id
                LEFT JOIN (SELECT SUM(m.price) AS total, m.id_contract, m.tgl_input, m.tgl_selesai FROM transaction_sample m GROUP BY m.id_contract) AS sampleprice ON sampleprice.id_contract = a.contract_id
                INNER JOIN mstr_transaction_kontrakuji c ON c.id_kontrakuji = a.contract_id
                INNER JOIN payment_contract d ON d.id_contract = c.id_kontrakuji
                INNER JOIN mstr_products_contactcategory e ON e.id = c.id_contract_category
                ...
                LEFT JOIN mstr_customers_contactperson h ON h.id_cp = f.id_cp
                INNER JOIN hris_employee i ON i.user_id = a.user_id
                WHERE a.position = 1
                AND a.sample_id = 0
                ' . $z . '
                AND c.id_kontrakuji IS NOT NULL
                AND DATE_FORMAT(a.inserted_at,"%Y-%m-%d") BETWEEN "' . $request->input('from') . '" AND "' . $request->input('to') . '"
            ...
            //     b.no_po,
            //     b.no_penawaran,
            //     SUM(a.price) as totalpembayaransample,
            //     g.discount_lepas,
            //     (SUM(a.price) - g.discount_lepas) as subtotal,
            //     c.inserted_at,
            //     c.status,
            //     b.created_at as tglbuat
            // ')
            // ->leftJoin('mstr_transaction_kontrakuji as b','b.id_kontrakuji','a.id_contract')
            // ->leftJoin('condition_contracts as c','c.contract_id','b.id_kontrakuji')
            // ->leftJoin('mstr_products_contactcategory as z','z.id','b.id_contract_category')
            // ->leftJoin(\DB::raw('( SELECT SUM(total) as totalsampling, id_transaction_contract from transaction_sampling_contract group by id_transaction_contract) as samplingtotal'),'samplingtotal.id_transaction_contract','b.id_kontrakuji')
            // ->leftJoin(\DB::raw('( SELECT SUM(total) as totalakg, id_transaction_kontrakuji from transaction_akg_contract group by id_transaction_kontrakuji) as akgtotal'),'akgtotal.id_transaction_kontrakuji','b.id_kontrakuji')
            // ->leftJoin('mstr_customers_handle as d','d.idch','b.id_customers_handle')
            // ->leftJoin('mstr_customers_customer as e','e.id_customer','d.id_customer')
            // ->leftJoin('mstr_customers_contactperson as f','f.id_cp','d.id_cp')
            // ->leftJoin('payment_contract as g','g.id_contract','a.id_contract')
            // ->leftJoin('hris_employee as h','h.user_id','c.user_id')
            // ->where('c.position',1)
            // ->whereBetween(\DB::raw('DATE_FORMAT(c.inserted_at,"%Y-%m-%d")'),[$request->input('from'), $request->input('to')])
            // ->groupBy('a.id_contract')->get();
