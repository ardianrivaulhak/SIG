...
import { ModalInfoComponent } from "app/main/analystpro/master/customers/modal-info/modal-info.component";
...
export class InvoiceFinanceComponent implements OnInit {
  ...
  {
    let b = [];
    let a = this.datainvoice.filter((x) => 
      {
        x.format == 4 && x.invoice_condition[0].status == 3 ? b = b.concat(x) : ''
      })
      this.countInvoice = b.length
  }
  ...
  openPdf(v,val) {
    ...
    getInfo(id) {
      let dialogCust = this.dialog.open(ModalInfoComponent, {
          height: "auto",
          width: "500px",
          data: {
              data: id,
