...
@Component({
    selector: "app-customertaxaddress-det",
    templateUrl: "./customertaxaddress-det.component.html",
    styleUrls: ["./customertaxaddress-det.component.scss"],
    animations: fuseAnimations,
    encapsulation: ViewEncapsulation.None,
})
export class CustomertaxaddressDetComponent implements OnInit {
    ...
    constructor(
        ...
        private _custAddress: CustomertaxaddressService
    ) {
    ...
    async getDataCust() {
        await this._custServ
            .getDataCustomers(this.datacustomer)
            .then((x) => {
                console.log(x["data"]);
    ...
    saveForm() {
        ...
        this._custAddress.updateData(a).then((y) => {
            ...
            setTimeout(() => {
                ...
                this._route.navigateByUrl("analystpro/customer-taxaddress");
                this.load = false;
            }, 2000);
        });
    }
    ...
    getdatadetail() {
        this._custAddress
            .getDataDetail(this.idCustomerAddress)
            .then((x) => {
            ...
            .then(async () => {
                this.customersdata = await this.customersdata.concat(
                    this.detaildata[0].customers
                );
                console.log(this.customersdata);
            })
            .then(
                () =>
                    (this.customerid = this.idCustomerAddress
                        ? this.detaildata[0].customers.id_customer
                        : null)
            )
            .then(
                () =>
                    (this.alamatid = this.idCustomerAddress
                        ? this.detaildata[0].address
                        : null)
            );
    }
