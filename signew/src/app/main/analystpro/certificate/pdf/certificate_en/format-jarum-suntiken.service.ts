...
export class FormatJarumSuntikenService {
  ...
  async generatePdf(sample, check){
    ...
    await sample.parameters.forEach( (x, i) => {
        ...
        `${x.unit == null ? '-' : x.unit}`,
          x.simplo == null ? '-' : x.simplo.includes('^') ? {
            alignment: 'center',
            margin:[ 18, 0, 0, 0],
            columns: [                    
            {text: x.simplo.split('^')[0],alignment: 'right'},
            {text: x.simplo.split('^')[1],alignment: 'left', margin:[-5, 0, 0, 0],  fontSize: 6},
          ]} : {
          text: x.simplo
          },
          x.duplo == null ? '-' : x.duplo.includes('^') ? {
            alignment: 'center',
            margin:[ 18, 0, 0, 0],
            columns: [                    
            {text: x.duplo.split('^')[0],alignment: 'right'},
            {text: x.duplo.split('^')[1],alignment: 'left', margin:[-5, 0, 0, 0],  fontSize: 6},
          ]} : {
          text: x.duplo
          },
          x.triplo == null ? '-' : x.triplo.includes('^') ? {
            alignment: 'center',
            margin:[ 18, 0, 0, 0],
            columns: [                    
            {text: x.triplo.split('^')[0],alignment: 'right'},
            {text: x.triplo.split('^')[1],alignment: 'left', margin:[-5, 0, 0, 0],  fontSize: 6},
          ]} : {
          text: x.triplo
          },
        `${x.standart == null ? '-' : x.standart}`,
    ...
    let tableParameter = await {
      ...
      table: {
          widths: [15, 70, 30, 40, 40, 40, 60, 50, 70],
          body: rows,
          dontBreakRows: true,
      },
      alignment: "center",
      ...
      content: 
          ...
          },      
        {
            ...
            table: {
              widths: [15, 250, 5, "*"],
              body: [
                  [{}, {text: '4.1. Requirement'}, {}, {}],
                  [{}, {text:'\u200B\t \u200B\t KMK No. 01.07/Menkes/1126/2022 About Compendium Of Health Devices', colSpan: 3}, {}, {}],
                  [{}, {text: '4.2. Sample Infomation'}, {}, {}],
                  [{}, {text:'\u200B\t \u200B\t 4.2.1. Sample Name'}, ':', sample.sample_name == null ? '-' :  sample.sample_name ],
                  [{}, {text:'\u200B\t \u200B\t 4.2.2. Sample Number'}, ':', sample.no_sample == null ? '-' :  sample.no_sample ],
                  [{}, {text:'\u200B\t \u200B\t 4.2.3. Marketing Number'}, ':', sample.transaction_sample.kontrakuji.contract_no],
                  [{}, {text: '4.3. General Requirement'}, {}, {}],
              ]
            }
        },
        tableParameter, 
        {
            ...
            table : {
                body: [
                    [{text: `Remark`, bold: true}, {text: `:`}, {text: `*According to Requirement (KMK No. 01.07/Menkes/1126/2022 About Compendium Of Health Devices)`}],
                ],
            }
        },
        {
                  ...
                  columns: [
                    sample.condition_cert[sample.condition_cert.length - 1].status != 3 ? 
                    {
                        ...
                        table: {
                            ...
                            width: 500,
                            body: [
                                [{text: `Bogor, ${sample.date_at == null ? '-' : moment(new Date(sample.date_at)).format("MMMM DD, YYYY")}`}],
                                [{text: `PT. Saraswanti Indo Genetech`}],
                                [{
                                  image: image.dwi,
                                  width: 150,
