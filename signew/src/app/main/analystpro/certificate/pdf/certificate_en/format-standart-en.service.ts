...
export class FormatStandartEnService {
  pdfMake: any;
  ...
  async generatePdf(sample, check){
    ...
    x.standart == null ? '-' : x.standart.includes('^') ? {
        alignment: 'center',
        columns: [                    
        {text: x.standart.split('^')[0],alignment: 'right'},
        {text: x.standart.split('^')[1],alignment: 'left', margin:[-5, 0, 0, 0],  fontSize: 6},
            ]} : {
        text: x.standart
    },
          `${x.lod == null ? '-' : x.lod}`,
